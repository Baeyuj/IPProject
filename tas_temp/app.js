...
var filepath = '/sys/bus/w1/devices/28-3ce1d4439e4f/temperature';

// Trigger a temperature measurement once per second
setInterval(() => {
	fs.readFile(filepath, 'utf-8', (err,data) => {
		...
		var temp = data / 1000.0;
		if (tas_state=='upload') {
 			for(var i = 0; i < upload_arr.length; i++) {
                		if(upload_arr[i].id != "temp") {
                    			var cin = {ctname: upload_arr[i].ctname, con: temp};
