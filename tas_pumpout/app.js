...
function fetchCinFromMobius() {
    const options = {
        ...
        port: 7579,
        path: '/Mobius/IoTPTeam2/pumpout/la',  // 'la'는 latest content instance
        method: 'GET',
        headers: {
            'X-M2M-RI': '12345',
...
function control_pump(comm_num) {
    ...
    if (action === 1) {
        console.log('Turning on the out water pump.');
        OUT_ON_PIN.writeSync(1); // GPIO 핀을 HIGH로 설정하여 펌프를 켭니다.
    } else if (action === 0) {
        console.log('Turning off the out water pump.');
        OUT_ON_PIN.writeSync(0); // GPIO 핀을 LOW로 설정하여 펌프를 끕니다.
    } else {
